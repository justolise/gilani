...
export function CurriculumSubjectBar({
  ...
  return (
    <div className={`w-full flex flex-col items-center gap-2 ${className}`}>
      <div className="flex items-center gap-1.5 text-sm sm:text-xs text-muted-foreground/80 font-medium">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 max-w-2xl px-2">
        {config.subjects.map((subj: CurriculumSubject) => (
          <button
            ...
            className={`group inline-flex items-center gap-2 rounded-full border px-3.5 py-2 text-sm sm:text-xs font-medium transition-all duration-200 min-h-[38px] ${
              disabled
